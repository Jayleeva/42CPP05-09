...
std::deque<unsigned int>	jacobsthalMerge(std::deque<unsigned int> main, std::deque<unsigned int> pending, size_t blockSize, std::deque<size_t> &jacobsthal, std::deque<unsigned int> container)
{
	...
	while (pending_nblocks >= difjac)
	{
		while (difjac > 0)
		{
			...
			binaryInsert(main, main.begin(), elend, blockSize, max_);
			pending_nblocks --;
			difjac --;
		}
	...
	while (pending_nblocks > 0)
	{
		...
		/*else
			max_ = ?*/
		elend = pending.begin() + size / blockSize;
		std::cout << "[DEQ] standard ; pending.begin() + size / blockSize = " << *(pending.begin() + size / blockSize) << std::endl;
		binaryInsert(main, main.begin(), elend, blockSize, max_);
		size -= size / blockSize;
		i ++;
		pending_nblocks --;
